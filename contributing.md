...
### 1\. Otimização do Modelo (`services/ml/petr4_tuning.py`)
...
Se você alterar a arquitetura (e.g., número de camadas LSTM, *dropout*, *epochs*):
  ...
  * **Atualize Artefatos:** Se o treinamento for bem-sucedido, substitua os arquivos `tuned_model.pth` e `scaler.joblib` na pasta `model_pipeline/artifacts/` e **inclua-os no PR**.
...
### 3\. Integração na API (`app/services/ml/petr4_predictor_lit.py`)
...
| `app/services/ml/*.py` | Lógica de **Treinamento, Otimização, Carregamento e Predição** do modelo LSTM. |
| `model_pipeline/lstm.ipynb` | **Treinamento inicial e Arquitetura** do modelo LSTM. |
